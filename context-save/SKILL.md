...
Set the artifact path and title from the checkpoint just written, then publish:

```bash
ISSUE_ARTIFACT_PATH="$FILE"
ISSUE_ARTIFACT_TITLE="Context: $TITLE_SLUG"
```
...
ISSUE_URL=$(~/.claude/skills/gstack/bin/gstack-issue-artifact create --kind gstack:context-save \
...
echo "[issue-artifacts] published gstack:context-save -> $ISSUE_URL"
```
<!-- @issue-artifacts:end -->
