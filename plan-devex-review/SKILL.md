...
Set the artifact path and title from the plan file just reviewed, then publish:

```bash
ISSUE_ARTIFACT_PATH="$DESIGN"
ISSUE_ARTIFACT_TITLE="DX plan: $(basename "$DESIGN" .md | sed 's/^[0-9]*-//')"
```
...
ISSUE_URL=$(~/.claude/skills/gstack/bin/gstack-issue-artifact create --kind gstack:devex-plan \
...
echo "[issue-artifacts] published gstack:devex-plan -> $ISSUE_URL"
```
<!-- @issue-artifacts:end -->
