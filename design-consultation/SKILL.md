...
Set the artifact path and title from the DESIGN.md just written, then publish:

```bash
ISSUE_ARTIFACT_PATH="$(git rev-parse --show-toplevel 2>/dev/null || pwd)/DESIGN.md"
ISSUE_ARTIFACT_TITLE="Design system: $(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")"
```
...
ISSUE_URL=$(~/.claude/skills/gstack/bin/gstack-issue-artifact create --kind gstack:design-doc --label design-system \
...
echo "[issue-artifacts] published gstack:design-doc -> $ISSUE_URL"
```
<!-- @issue-artifacts:end -->
